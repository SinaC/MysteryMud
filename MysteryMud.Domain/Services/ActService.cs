...
public class ActService : IActService
{
    ...
    public string FormatFor(EntityId viewer, string format, params object[] args)
        => FormatActOneLine(viewer, format, args);

    private string FormatActOneLine(EntityId target, string format, params object[]? arguments)
    {
    ...
    // TODO Ability
    //      ability name
    private void FormatActOneArgument(EntityId target, StringBuilder sb, int argumentIndex, ReadOnlySpan<char> argumentFormat, object? argument)
    {
        if (argument is EntityId entity && !_world.IsAlive(entity))
            return;

        if (argument is EntityId character && _world.Has<CharacterTag>(character))
        {
            var letter = argumentFormat.Length > 0 ? argumentFormat[0] : (argumentIndex == 0 ? 'N' : 'n'); // default to 'N' or 'n' if no format specified
            ref var gender = ref _world.TryGetRef<Gender>(character, out var hasGender);
            var genderKind = hasGender ? gender.Value : GenderKind.Neutral;
            switch (letter)
            {
                case 'p':
                    if (target == character) sb.Append("your");
                    else AppendPossessive(sb, RelativeDisplayName(_world, character, target));
                    break;
                case 'P':
                    if (target == character) sb.Append("Your");
                    else AppendPossessive(sb, RelativeDisplayName(_world, character, target), true);
                    break;
                case 'n':
                    sb.Append(target == character ? "you" : RelativeDisplayName(_world, character, target));
                    break;
                case 'N':
                    sb.Append(target == character ? "You" : RelativeDisplayName(_world, character, target).FirstCharToUpper());
                    break;
                case 'e':
                ...
                case 'y':
                    if (target == character) sb.Append("your");
                    else sb.Append(RelativeDisplayName(_world, character, target)).Append("'s");
                    break;
                case 'Y':
                    if (target == character) sb.Append("Your");
                    else sb.Append(RelativeDisplayName(_world, character, target)).Append("'s");
        ...
        if (argument is EntityId item && _world.Has<ItemTag>(item))
        {
            var letter = argumentFormat.Length > 0 ? argumentFormat[0] : (argumentIndex == 0 ? 'N' : 'n'); // default to 'N' or 'n' if no format specified
            ...
            {
                case 'p':
                    AppendPossessive(sb, RelativeDisplayName(_world, item, target));
                    break;
                case 'P':
                    AppendPossessive(sb, RelativeDisplayName(_world, item, target), true);
                    break;
                case 'n':
                    sb.Append(RelativeDisplayName(_world, item, target));
                    break;
                case 'N':
                    sb.Append(RelativeDisplayName(_world, item, target).FirstCharToUpper());
                    break;
                case 'e':
                ...
                case 'F':
                    ...
                    break;
                case 'y':
                    sb.Append(RelativeDisplayName(_world, item, target)).Append("'s");
                    break;
                case 'Y':
                    sb.Append(RelativeDisplayName(_world, item, target)).Append("'s");
                    break;
        ...
        if (argument is EntityId room && _world.Has<Room>(room))
        {
            sb.Append(EntityHelpers.DisplayName(_world, room));
            return;
        }
