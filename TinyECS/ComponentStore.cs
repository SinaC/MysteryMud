...
public sealed class ComponentStore<T> : World.IComponentStore
{
    ...
    // Maps entity *index* to position in the dense array.
    ...
    /// <summary>Returns true if the entity has this component and its generation is current.</summary>
    ...
    {
        if (Has(entity))
            throw new InvalidOperationException($"{entity} does already have component {typeof(T).Name}");
    ...
    /// <summary>Removes all components for this entity (same as Remove but no return value).</summary>
    ...
    object World.IComponentStore.AddRaw(EntityId entity, object value)
    {
        // optional: validate type
        if (value is not T typedValue)
            throw new InvalidCastException(
                $"Expected {typeof(T)}, got {value?.GetType()}"
            );

        ref var added = ref Add(entity, typedValue);
        return added!;
    }
    ...
    /// <summary>
    /// Returns the entity at a known dense position.
    /// Used by WorldQueryExtensions to avoid re-deriving the pivot position
    /// through the sparse array when we already know the loop index.
    /// </summary>
    internal EntityId EntityAt(int pos) => _dense[pos];

    /// <summary>
    /// Returns the position of <paramref name="entity"/> in the dense array,
    /// or -1 if absent / stale.  One sparse read — no dictionary lookup.
    /// </summary>
    internal int GetDenseIndex(EntityId entity)
    {
        uint idx = entity.Index;
        if (idx >= _sparse.Length) return NO_ENTRY;
        int pos = _sparse[idx];
        return (pos != NO_ENTRY && _dense[pos] == entity) ? pos : NO_ENTRY;
    }

    /// <summary>
    /// Returns the dense position of <paramref name="entity"/>, or NO_ENTRY if absent.
    /// Skips the generation re-check — safe only when the entity is known live
    /// (e.g. it just came from iterating another store's dense array).
    /// Saves one array read per non-pivot component in the hot loop.
    /// </summary>
    internal int GetDenseIndexFast(EntityId entity)
    {
        uint idx = entity.Index;
        if (idx >= _sparse.Length) return NO_ENTRY;
        return _sparse[idx];   // NO_ENTRY when absent, dense position otherwise
    }

    /// <summary>
    /// Returns a ref to the component at a known dense position.
    /// Caller must have validated the index via <see cref="GetDenseIndex"/> first.
    /// </summary>
    ...
    public ref struct Enumerator
    {
        ...
        public EntityId Entity => _store._dense[_index];
