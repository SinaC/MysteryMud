...
public class OstatCommand : ICommand
{
    ...
    private Entity? SearchTarget(Entity actor, ref TargetSpec targetSpec)
    {
        ...
        if (hasInventory)
        {
            var inventoryItem = EntityFinder.SelectSingleTarget(actor, targetSpec, inventory.Items);
        ...
        var roomItem = EntityFinder.SelectSingleTarget(actor, targetSpec, roomItems);
